...
###
# 削除閾値となる日付を取得
$td = (get-date).addDays(-365)#削除対象の日数分減算
$pastDate = get-date($td) -Format yyyyMMdd
$myfolderpath = "D:\coreif_work\集信"
$myfolder = get-childItem $myfolderpath -Filter *.DAT #DATファイルに絞っておく
...
#$pastDate = get-date($td) -Format yyyyMMdd
$myfolderpath = "D:\coreif_work\集信異常"
$myfolder = get-childItem $myfolderpath -Filter *.DAT #DATファイルに絞っておく
foreach($myfile in $myfolder){
    #入力ファイルの更新日付を取得
    $mytimestamp = $(get-itemProperty $myfolderpath\$myfile).LastWriteTime.ToString('yyyyMMdd')
...
#$pastDate = get-date($td) -Format yyyyMMdd
$myfolderpath = "D:\coreif_work\配信異常"
$myfolder = get-childItem $myfolderpath -Filter *.DAT #DATファイルに絞っておく
foreach($myfile in $myfolder){
    #入力ファイルの更新日付を取得
    $mytimestamp = $(get-itemProperty $myfolderpath\$myfile).LastWriteTime.ToString('yyyyMMdd')
