...
namespace RbsMain
{
    public class 正梱投入指示
    {
        ...
        public 正梱投入指示(RbsMain paraRbsMain, CmnObject paraCmnObj) {
            RbsMain = paraRbsMain;
            CmnObj = paraCmnObj;
        }

        public void 正梱投入指示Func() {
            for (; ; )
            {
                ...
                for (; ; )
                {
                    try {
                        ...
                        {
                            if (CmnObj.正梱投入指示Queue.Count > 0) {
                                QueueData = CmnObj.正梱投入指示Queue.Dequeue();   // キューから取り出す（先入先出）
                            } else {
                                QueueData = "";
                            }
                        }
                        if (String.IsNullOrEmpty(QueueData)) {
                            #region 開始イベント待ち
                            CmnObj.正梱投入指示Event.WaitOne();
                            CmnObj.正梱投入指示Event.Reset();
                            #endregion
                        } else {
                        ...
                        RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                            , new object[] { "正梱投入指示キューからの取り出し例外：" + ex.Message.ToString() });
                        continue;
                    }
                }
                #endregion
                ...
                if (ReqData[CmnObject.QUE種別] == CmnObject.QUE_抽出要求) {
                    CmnObj.正梱投入指示処理中フラグ = true;

                    #region 配信データSEQ取得（ゼロ件の場合は後継処理スルー）
                    配信データSEQ = 0;
                    string retStr = CmnObj.配信データSEQ取得(CmnObj.HulftID正梱投入指示, ref 配信データSEQ);
                    if (retStr != "") {
                        RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                            , new object[] { "正梱投入指示：配信データSEQ取得例外 " + retStr });
                        配信データSEQ = 0;
                    }
                    if (配信データSEQ == 0) {
                        CmnObj.正梱投入指示処理中フラグ = false;
                        continue;
                    }
                    CmnObj.正梱投入指示_配信SEQ = 配信データSEQ;
                    #endregion
                    ...
                    RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                        , new object[] { "-----------------------------------------------------------------" });
                    RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                        , new object[] { "正梱投入指示：1.配信開始 [IF_SND_FULL_INPUT_ORDER]" });
                    #endregion
                    ...
                    using (var connection = new SqlConnection(connectionString)) {
                        ...
                        try {
                            ...
                            RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                                , new object[] { "正梱投入指示：DBトランザクション例外 " + ex.Message });
                            CmnObj.正梱投入指示処理中フラグ = false;
                            continue;
                        }
                        #endregion
                        ...
                        using (var tran = connection.BeginTransaction()) {
                            try {
                                ...
                                using (var command = new SqlCommand() { Connection = connection, Transaction = tran, CommandType = CommandType.StoredProcedure }) {
                                    ...
                                    command.CommandText = "[prep_IF_SND_FULL_INPUT_ORDER]";
                                    ...
                                    if (ret == 0 && 配信データ件数 > 0) {
                                        ...
                                        if (ret != 0) {
                                            RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                                                , new object[] { "正梱投入指示：IF抽出ストアド例外 " + retStr });
                                        } else {
                                            RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                                                , new object[] { "正梱投入指示：IF抽出ストアド 配信データ件数ゼロ" });
                                        }
                                        ...
                                        RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                                            , new object[] { "正梱投入指示：IF抽出ロールバック実行" });
                                ...
                                RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                                    , new object[] { "正梱投入指示：IF抽出トランザクション例外 " + ex.Message });
                        ...
                        #region 2.配信データ件数ログ
                        RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                            , new object[] { "正梱投入指示：2.配信データ件数->" + 配信データ件数.ToString() });
                        #endregion

                        // 抽出データテーブルクリア
                        CmnObj.dt_RBSD010.Clear();

                        // 抽出データセット
                        using (var command = connection.CreateCommand()) {
                            try {
                                command.CommandText =
                                    @"SELECT * FROM IF_RAW_SND_FULL_INPUT_ORDER ORDER BY SEQ";
                                var adapter = new SqlDataAdapter(command);
                                adapter.Fill(CmnObj.dt_RBSD010);
                            } catch (Exception ex) {
                                RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                                    , new object[] { "正梱投入指示：配信データ取得例外 " + ex.Message });

                                #region 配信データ削除（例外対応）
                                retStr = CmnObj.配信データ削除(CmnObj.HulftID正梱投入指示);
                                if (retStr != "") {
                                    RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                                        , new object[] { "正梱投入指示：配信データ削除例外 " + retStr });
                                }
                                #endregion
                        ...
                        // データテーブルコミット
                        CmnObj.dt_RBSD010.AcceptChanges();
                    }
                    #endregion

                    #region 配信ファイル作成
                    ファイル名 = CmnObj.HulftID正梱投入指示 + ".csv";
                    ...
                    try {
                        using (StreamWriter sw = new StreamWriter(配信ファイル, false, Encoding.GetEncoding("Shift_JIS"))) {
                            ...
                            foreach (DataRow dr in CmnObj.dt_RBSD010.Rows) {
                                if ((string)dr[0] == "L")            // 1階層目
                                {
                                    for (int i = 1; i < 8; i++)     // 階層識別子を除外する
                                    {
                        ...
                        RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                            , new object[] { "正梱投入指示：配信ファイル作成例外 " + ex.Message });
                    ...
                    #region 3.配信ファイル作成ログ
                    RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                        , new object[] { "正梱投入指示：3.配信ファイル作成 [" + ファイル名 + "]" });
                    #endregion

                    #region HULFT配信要求(EXE起動終了するまで待つ)
                    // 引数セット(第1引数:起動区分,第2引数:ファイルID)
                    string strCmd = " -f " + CmnObj.HulftID正梱投入指示;
                    try {
                        ...
                        RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                            , new object[] { "正梱投入指示：4.HULFT配信実行 [" + ファイル名 + "]" });
                        #endregion
                    } catch (Exception ex) {
                        RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                            , new object[] { "正梱投入指示：HULFT配信実行例外 " + ex.Message });
                    ...
                    using (var connection = new SqlConnection(connectionString)) {
                        ...
                        using (var tran = connection.BeginTransaction()) {
                            try {
                                ...
                                RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                                    , new object[] { "正梱投入指示：5.IFデータ退避削除開始" });
                                #endregion
                                ...
                                using (var command = new SqlCommand() { Connection = connection, Transaction = tran, CommandType = CommandType.StoredProcedure }) {
                                    ...
                                    // INPUTの設定
                                    command.Parameters.Add("@FILEID", System.Data.SqlDbType.VarChar).Value = CmnObj.HulftID正梱投入指示;
                                    ...
                                    if (ret == 0) {
                                        ...
                                        RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                                            , new object[] { "正梱投入指示：IF退避削除ストアド例外 " + retStr });

                                        // ロールバック
                                        tran.Rollback();

                                        RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                                            , new object[] { "正梱投入指示：IF退避削除ロールバック実行" });
                                    }
                                }

                                #region 6.IFデータ退避削除完了ログ
                                RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                                    , new object[] { "正梱投入指示：6.IFデータ退避削除完了" });
                                #endregion
                            } catch (Exception ex) {
                                RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                                    , new object[] { "正梱投入指示：IF退避削除トランザクション例外 " + ex.Message });
                    ...
                    if (CmnObj.正梱投入指示_配信SEQ != 0) {
                        ファイル名 = timestamp + "_" + CmnObj.HulftID正梱投入指示 + ".csv";
                        CmnObj.配信ファイル名登録1(ファイル名, CmnObj.正梱投入指示_配信SEQ, "H");
                        CmnObj.配信ファイル名登録1(ファイル名, CmnObj.正梱投入指示_配信SEQ, "D1");
                        CmnObj.正梱投入指示_配信SEQ = 0;
                    }
                    #endregion
                    ...
                    ファイル名 = CmnObj.HulftID正梱投入指示 + ".csv";
                    配信ファイル = CmnObj.配信フォルダ + ファイル名;
                    保存ファイル = CmnObj.保存フォルダ_RBSD010 + timestamp + "_" + ファイル名;
                    ...
                    if (exMsg != "") {
                        RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                            , new object[] { "正梱投入指示：移動(Copy)例外 " + exMsg });
                    }
                    for (int i = 0; i < 10; i++) {
                        try {
                            File.Delete(配信ファイル);
                    ...
                    if (exMsg != "") {
                        RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                            , new object[] { "正梱投入指示：移動(Delete)例外 " + exMsg });
                    }
                    #endregion

                    #region 8.配信バックアップログ
                    RbsMain.Invoke(new dlgAccTrace(RbsMain.AccTrace)
                    , new object[] { "正梱投入指示：8.配信バックアップ [" + ファイル名 + "]" });
                    #endregion
                }
            }
