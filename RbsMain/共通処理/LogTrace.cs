...
namespace RbsMain
{
	...
	public class DailyLoggingTraceListener : TraceListener
	{
        ...
        /// <summary>データベース接続文字列を取得または設定します。</summary>
        public string DBcmndString { get; set; }
		...
		public override void WriteLine(string message)
		{
            ...
            if (message == "起動時データベース接続異常")
            {
                return;
            }

            if (message.Contains("異常") || message.Contains("例外"))
            {
                #region 発生中アラーム登録
                using (var connection = new SqlConnection(DBcmndString))
                {
                    #region DB接続
                    try
                    {
                    ...
                    #region メッセージ切り出し（MAX5120byte）
                    string alarmMessage;
                    try
                    {
                        System.Text.Encoding sjis = System.Text.Encoding.GetEncoding("shift_jis");
                        byte[] b = sjis.GetBytes(message);
                        int len = b.Length;
                        if (len > 5120)
                        {
                            alarmMessage = sjis.GetString(b, 0, 5120);
                        }
                        else
                        {
                            alarmMessage = sjis.GetString(b, 0, len);
                        }
                    }
                    catch
                    {
                    ...
                    using (var tran = connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            using (var command = new SqlCommand() { Connection = connection, Transaction = tran, CommandType = CommandType.StoredProcedure })
                            {
                                ...
                                command.CommandText = "[post_CURRENT_ALARMS]";
                                command.Parameters.Clear();

                                // INPUTの設定
                                command.Parameters.Add("@CATEGORY", System.Data.SqlDbType.VarChar).Value = "RBS";
                                command.Parameters.Add("@MES", System.Data.SqlDbType.VarChar).Value = alarmMessage;
