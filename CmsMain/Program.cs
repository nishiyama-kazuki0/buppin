...
namespace CmsMain
{
    internal static class Program
    {
        ...
        {
            //Mutexクラスの作成
            _mutex = new System.Threading.Mutex(false, @"Global\CMSIFAPP");

            //ミューテックスの所有権を要求する
            if (_mutex.WaitOne(0, false) == false)
            {
                //すでに起動していると判断する
                MessageBox.Show("多重起動はできません。", "CMSインターフェース起動失敗",
        ...
        {
            // 上書き false 追記 true
            using (StreamWriter sw = new StreamWriter(@"C:\CMSIF\CMSIF_ERROR.LOG", false, Encoding.GetEncoding("Shift_JIS")))
            {
                sw.Write("\n\n————————\n" + extraMessage + "\n————————\n" +
                            "【エラー内容】\n" + ex.Message + "\n" +
                            "【スタックトレース】\n" + ex.StackTrace);
