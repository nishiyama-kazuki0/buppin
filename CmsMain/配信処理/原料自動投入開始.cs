...
namespace CmsMain
{
	public class 原料自動投入開始
	{
        ...
        public 原料自動投入開始(CmsMain paraCmsMain, CmnObject paraCmnObj)
		{
			CmsMain = paraCmsMain;
			CmnObj = paraCmnObj;
		}

		public void 原料自動投入開始Func()
		{
			for (; ; )
			{
				...
				for (; ; )
				{
					try
					{
						...
						{
							if (CmnObj.原料自動投入開始Queue.Count > 0)
							{
								QueueData = CmnObj.原料自動投入開始Queue.Dequeue();	// キューから取り出す（先入先出）
							}
							else
                            {
								QueueData = "";
						...
						{
							#region 開始イベント待ち
							CmnObj.原料自動投入開始Event.WaitOne();
							CmnObj.原料自動投入開始Event.Reset();
							#endregion
						}
						else
                        {
					...
					catch (Exception ex)
					{
						CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
							, new object[] { "原料自動投入開始キューからの取り出し例外：" + ex.Message.ToString() });
						continue;
					}
				}
				#endregion
				...
				{
					CmnObj.原料自動投入開始処理中フラグ = true;

                    #region 配信データSEQ取得（ゼロ件の場合は後継処理スルー）
                    配信データSEQ = 0;
					string retStr = CmnObj.配信データSEQ取得(CmnObj.HulftID原料自動投入開始, ref 配信データSEQ);
					if (retStr != "")
					{
						CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
							, new object[] { "原料自動投入開始：配信データSEQ取得例外 " + retStr });
						配信データSEQ = 0;
					}
					if (配信データSEQ == 0)
					{
						CmnObj.原料自動投入開始処理中フラグ = false;
						continue;
					}
                    CmnObj.原料自動投入開始_配信SEQ = 配信データSEQ;
                    #endregion
                    ...
                    CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
						, new object[] { "-----------------------------------------------------------------" });
					CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
						, new object[] { "原料自動投入開始：1.配信開始 [IF_SND_AUTO_INPUT_START]" });
					#endregion
					...
					using (var connection = new SqlConnection(connectionString))
					{
						...
						{
							CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
								, new object[] { "原料自動投入開始：DBトランザクション例外 " + ex.Message });
							CmnObj.原料自動投入開始処理中フラグ = false;
							continue;
						}
						#endregion
						...
						using (var tran = connection.BeginTransaction())
						{
							try
							{
								...
								using (var command = new SqlCommand() { Connection = connection, Transaction = tran, CommandType = CommandType.StoredProcedure })
								{
									...
									command.CommandText = "[prep_IF_SND_AUTO_INPUT_START]";
									...
									else
									{
										...
										{
											CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
												, new object[] { "原料自動投入開始：IF抽出ストアド例外 " + retStr });
										}
										else
										{
											CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
												, new object[] { "原料自動投入開始：IF抽出ストアド 配信データ件数ゼロ" });
										}
										...
										CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
											, new object[] { "原料自動投入開始：IF抽出ロールバック実行" });
							...
							{
								CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
									, new object[] { "原料自動投入開始：IF抽出トランザクション例外 " + ex.Message });
                        ...
                        #region 2.配信データ件数ログ
                        CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
							, new object[] { "原料自動投入開始：2.配信データ件数->" + 配信データ件数.ToString() });
						#endregion

						// 抽出データテーブルクリア
						CmnObj.dt_MTLU010.Clear();
						...
						using (var command = connection.CreateCommand())
						{
							...
							{
								command.CommandText =
                                    @"SELECT * FROM IF_RAW_SND_AUTO_INPUT_START ORDER BY SEQ";
								var adapter = new SqlDataAdapter(command);
								adapter.Fill(CmnObj.dt_MTLU010);
							}
							catch (Exception ex)
							{
								CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
									, new object[] { "原料自動投入開始：配信データ取得例外 " + ex.Message });

								#region 配信データ削除（例外対応）
								retStr = CmnObj.配信データ削除(CmnObj.HulftID原料自動投入開始);
								if (retStr != "")
								{
									CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
										, new object[] { "原料自動投入開始：配信データ削除例外 " + retStr });
								}
								#endregion
						...
						// データテーブルコミット
						CmnObj.dt_MTLU010.AcceptChanges();
					}
                    #endregion

                    #region 配信ファイル作成
                    ファイル名 = CmnObj.HulftID原料自動投入開始 + ".csv";
					...
					try
					{
						using (StreamWriter sw = new StreamWriter(配信ファイル, false, Encoding.GetEncoding("Shift_JIS")))
						{
							...
							foreach (DataRow dr in CmnObj.dt_MTLU010.Rows)
							{
                                for (int i = 0; i < 4; i++)
								{
					...
					{
						CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
							, new object[] { "原料自動投入開始：配信ファイル作成例外 " + ex.Message });
					...
					#region 3.配信ファイル作成ログ
					CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
						, new object[] { "原料自動投入開始：3.配信ファイル作成 [" + ファイル名 + "]" });
					#endregion

					#region HULFT配信要求(EXE起動終了するまで待つ)
					// 引数セット(第1引数:起動区分,第2引数:ファイルID)
					string strCmd = " -f " + CmnObj.HulftID原料自動投入開始;
					try
					{
						...
						CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
							, new object[] { "原料自動投入開始：4.HULFT配信実行 [" + ファイル名 + "]" });
						#endregion
					}
					catch (Exception ex)
					{
						CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
							, new object[] { "原料自動投入開始：HULFT配信実行例外 " + ex.Message });
                    ...
                    using (var connection = new SqlConnection(connectionString))
                    {
                        ...
                        using (var tran = connection.BeginTransaction())
                        {
                            try
                            {
                                ...
                                CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
                                    , new object[] { "原料自動投入開始：5.IFデータ退避削除開始" });
                                #endregion
                                ...
                                using (var command = new SqlCommand() { Connection = connection, Transaction = tran, CommandType = CommandType.StoredProcedure })
                                {
                                    ...
                                    // INPUTの設定
                                    command.Parameters.Add("@FILEID", System.Data.SqlDbType.VarChar).Value = CmnObj.HulftID原料自動投入開始;
                                    ...
                                    {
                                        CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
                                            , new object[] { "原料自動投入開始：IF退避削除ストアド例外 " + retStr });

                                        // ロールバック
                                        tran.Rollback();

                                        CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
                                            , new object[] { "原料自動投入開始：IF退避削除ロールバック実行" });
                                    }
                                }

                                #region 6.IFデータ退避削除完了ログ
                                CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
                                    , new object[] { "原料自動投入開始：6.IFデータ退避削除完了" });
                                #endregion
                            }
                            catch (Exception ex)
                            {
                                CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
                                    , new object[] { "原料自動投入開始：IF退避削除トランザクション例外 " + ex.Message });
				...
				else if (ReqData[CmnObject.QUE種別] == CmnObject.QUE_抽出ファイルバックアップ要求)
				{
                    ...
                    if (CmnObj.原料自動投入開始_配信SEQ != 0)
					{
                        ファイル名 = timestamp + "_" + CmnObj.HulftID原料自動投入開始 + ".csv";
                        CmnObj.配信ファイル名登録1(ファイル名, CmnObj.原料自動投入開始_配信SEQ);
                        CmnObj.原料自動投入開始_配信SEQ = 0;
                    }
                    #endregion
                    ...
                    ファイル名 = CmnObj.HulftID原料自動投入開始 + ".csv";
                    配信ファイル = CmnObj.配信フォルダ + ファイル名;
                    保存ファイル = CmnObj.保存フォルダ_MTLU010 + timestamp + "_" + ファイル名;
                    ...
                    {
                        CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
                            , new object[] { "原料自動投入開始：移動(Copy)例外 " + exMsg });
                    }
                    for (int i = 0; i < 10; i++)
                    {
                        try
                    ...
                    {
                        CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
                            , new object[] { "原料自動投入開始：移動(Delete)例外 " + exMsg });
                    }
                    #endregion

                    #region 8.配信バックアップログ
                    CmsMain.Invoke(new dlgAccTrace(CmsMain.AccTrace)
                    , new object[] { "原料自動投入開始：8.配信バックアップ [" + ファイル名 + "]" });
                    #endregion
                }
            }
