...
/// </summary>
public partial class DataGridExtend4 : ComponentBase
{
    ...
    private async void OnRowDblClick(DataGridRowMouseEventArgs<IDictionary<string, object>> args)
    {
        SelectedData = (IList<IDictionary<string, object>>?)args;
        await SelectedDataChanged.InvokeAsync(SelectedData);
    }

    private async Task OnClickInternal()
    {
        await Click.InvokeAsync(Code);
    }
}