        ...
        try
        {
            ...
            if (_gridData != null && _gridData.Count() > 0)
            {
                ...
                if (_gridAllData != null && _gridAllData.Count() > 0)
                {
                    ...
                    _gridAllData[0][STR_GRID_COL_PRG_ピック進捗率] = _gridAllData[0][STR_GRID_COL_ピック進捗率] = CalcUtil.GetPercent(decピック済数, dec引当済数, 1);
                    _gridAllData[0][STR_GRID_COL_コーナー別仕分済数] = decコーナー別仕分済数;
                    _gridAllData[0][STR_GRID_COL_PRG_コーナー別仕分進捗率] = _gridAllData[0][STR_GRID_COL_コーナー別仕分進捗率] = CalcUtil.GetPercent(decコーナー別仕分済数, dec引当済数, 1);
                    _gridAllData[0][STR_GRID_COL_コーナー搬送済数] = decコーナー搬送済数;
                    _gridAllData[0][STR_GRID_COL_PRG_搬送進捗率] = _gridAllData[0][STR_GRID_COL_搬送進捗率] = CalcUtil.GetPercent(decコーナー搬送済数, dec引当済数, 1);
                }
            }
